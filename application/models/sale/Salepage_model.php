...
class Salepage_model extends CI_Model {
        ...
        public function Getuser_ordering()
               {

        $query = $this->db->query('SELECT uw.name,
          uw.user_id
           FROM user_owner as uw
           WHERE uw.owner_id="'.$_SESSION['owner_id'].'" AND uw.user_type="9"
        ...
        public function Getuser_ordering_table($data)
               {
        ...
        $query = $this->db->query('SELECT uw.name,
          ...
          ut.table_id
           FROM user_owner as uw
           LEFT JOIN user_ordering_table as ut on ut.user_id=uw.user_id
           WHERE uw.owner_id="'.$_SESSION['owner_id'].'" AND uw.user_type="9"
           AND ut.table_id="'.$data['table_id'].'"
           ORDER BY uw.user_id ASC');
           $encode_data = json_encode($query->result(),JSON_UNESCAPED_UNICODE );
           return $encode_data;
               ...
               public function Saveuser_ordering($data)
                       {
$this->db->query('DELETE FROM user_ordering_table WHERE table_id="'.$data['table_id'].'"');
               if ($this->db->insert("user_ordering_table", $data)){
                       return true;
                   }
        ...
        public function Discountcategory_ok($data)
       {
$query = $this->db->query('UPDATE sale_list_table
   SET product_price_discount=(product_price*'.$data['percent'].')/100
   WHERE product_category_id="'.$data['product_category_id'].'" and table_id="'.$data['table_id'].'" ');
...
$shift_end = $queryshiftend->result_array();
      ...
      //check sale_list_table if not have and clear to zoro
            $checksalelisttable = $this->db->query('SELECT *
            FROM sale_list_table ');
            $listtablehowmany = $checksalelisttable->num_rows();

      if($listtablehowmany == 0){
        $this->db->query('TRUNCATE TABLE sale_list_table');
        $this->db->query('TRUNCATE TABLE sale_list_order');
        $this->db->query('TRUNCATE TABLE sale_list_order_kitchen');
      }
...
$this->db->insert("sale_list_datail", $data);
  ...
  public function Findsalelistdub($data)
    {

  $query = $this->db->query('SELECT product_id,product_price,product_sale_num
...
$data2['user_ordering_id'] = $data['user_ordering_id'];
...
$dlst = $this->db->query('DELETE FROM sale_table_discount_last WHERE table_id="'.$data['table_id'].'"');
...
public function Deletelastsavesale($data)
{
  ...
  //start delete all menu in table
...
$querynum = $this->db->query('SELECT *, from_unixtime(adddate,"%d-%m-%Y %H:%i:%s") as adddate
    FROM sale_list_header
    WHERE owner_id="'.$_SESSION['owner_id'].'"
    AND shift_id="'.$_SESSION['shift_id'].'"
    AND sale_runno LIKE "%'.$data['searchtext'].'%"
    ORDER BY ID DESC LIMIT 30');


$query = $this->db->query('SELECT *,
  sh.user_id as user_id,
  sh.product_score_all as product_score_all,
  cw.product_score_all as cus_score,
  ...
  (SELECT name FROM user_owner as u WHERE u.user_id=sh.user_ordering_id) as name_user_ordering
    ...
    AND sh.shift_id="'.$_SESSION['shift_id'].'"
    AND sh.sale_runno LIKE "%'.$data['searchtext'].'%"
...
$this->db->insert("sale_list_table", $data);
...
$query = $this->db->query('SELECT  MAX(st.s_ID) as s_ID,
    ...
    st.user_id,
    st.status,
    st.product_sale_num as product_sale_num2,
     sum(st.product_sale_num) as product_sale_num,
    st.product_price,
    st.product_price_discount,
    (SELECT count(*) FROM sale_list_order as so WHERE so.product_name=st.product_name and so.table_id=st.table_id) as so_order
...
return $encode_data;
  ...
  public function Sale_table_discount_last($data)
          {

if($data['selecttable']==0){
  $this->db->query('DELETE FROM sale_table_discount_last WHERE table_id="'.$data['table_id'].'"');
  $this->db->insert("sale_table_discount_last", $data);
}


  $query = $this->db->query('SELECT * FROM sale_table_discount_last WHERE table_id="'.$data['table_id'].'"');
  ...
  $this->db->query('INSERT INTO sale_list_order_kitchen
     (s_ID,product_id,product_category_id,product_name,product_code,product_price,product_sale_num,product_price_discount,
...
public function Moveproductlisttable($data)
        {
          ...
          $this->db->query('INSERT INTO sale_list_table_move_log
             (old_table_id,new_table_id,mover,move_date,s_ID,product_id,product_category_id,product_name,product_code,product_price,product_sale_num,product_price_discount,product_score,note_order,adddate,owner_id,user_id,store_id,table_id,status)
            select "'.$data['old_table_id'].'","'.$data['new_table_id'].'","'.$_SESSION['name'].'","'.time().'",s_ID,product_id,product_category_id,product_name,product_code,product_price,product_sale_num,product_price_discount,product_score,note_order,adddate,owner_id,user_id,store_id,table_id,status from sale_list_table
...
$querymove = $this->db->query('UPDATE sale_list_table
    ...
    $query = $this->db->query('SELECT  MAX(st.s_ID) as s_ID,
        ...
        FROM sale_list_table as st
        WHERE st.owner_id="'.$_SESSION['owner_id'].'"
            AND st.table_id="'.$data['new_table_id'].'"
